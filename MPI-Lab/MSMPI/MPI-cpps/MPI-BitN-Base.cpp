...
int main() {
    ...
    ofstream resultFile("Bit-128");
    if (!resultFile) {
        cerr << "Failed to create the result file." << endl;
        return 1;
    }
    LARGE_INTEGER freq, start,startT,endT, end0;
    ...
    double elapsedSeconds = static_cast<double>(endT.QuadPart - startT.QuadPart) / freq.QuadPart;
    resultFile <<elapsedSeconds << endl;
    queryFile.close();
    resultFile.close();
